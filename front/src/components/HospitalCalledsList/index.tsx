...
export default function HospitalCalledsList() {
  const [filteredCalleds, setFilteresCalleds] = useState<CalledsProps[]>();
  const hospitalId = useAtomValue(hospitalIdFiltersAtom);
  ...
  useEffect(() => {
    const filtered = calleds?.filter((item) => item.CD_HOSPITAL === hospitalId);
    return setFilteresCalleds(filtered);
  }, [calleds, filteredCalleds, hospitalId]);
      ...
      <div>
        {filteredCalleds?.length && (
          <DataTable
            ...
            highlightOnHover
            columns={dataColumns}
            records={filteredCalleds}
            noRecordsText={"Nenhum dado encontrado"}
          />
        )}
      </div>
    </ScrollArea>
